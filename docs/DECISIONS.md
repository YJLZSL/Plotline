...
## ADR-004：时间轴用 Konva Canvas，而非 DOM

**状态**：已采纳  
**日期**：2026-06-22

### 理由
- PRD 性能要求"1000 事件流畅滚动"。DOM 节点数会爆炸。
- Canvas 渲染独立于 React reconciliation，无重渲染开销。
- react-konva 提供声明式 API，与 React 习惯一致。
- 天然支持拖拽、缩放、连线等画布交互。

### 后果
- 无法用浏览器原生 a11y（需手动添加键盘焦点）。
- 截图导出需用 `stage.toDataURL()`。
