...
## ADR-023：AI 助手消息采用 react-markdown 渲染 Markdown

**状态**：已采纳  
**日期**：2026-06-27

### 背景
v2.7.0 Task 2 修复 AI 助手面板中 `**粗体**`、`*斜体*`、`` `代码` `` 等 Markdown 语法被直接显示为原始字符的问题。

候选方案：
- `react-markdown` + `remark-breaks` + `rehype-sanitize`
- `marked` + `DOMPurify` + `dangerouslySetInnerHTML`
- 自研轻量 Markdown 解析器

### 决策
选择 **`react-markdown` + `remark-breaks` + `rehype-sanitize`**。

### 理由
- React 原生集成，无需 `dangerouslySetInnerHTML`，避免 XSS 风险。
- `remark-breaks` 保留单换行，与聊天消息中 AI 输出的段落换行习惯一致。
- `rehype-sanitize` 作为第二道防线，过滤非法 HTML/属性。
- 组件化渲染：通过 `components` 属性把 `strong`/`em`/`code` 等映射到 Tailwind 主题类名，保持 UI 一致。

### 后果
- 新增依赖：`react-markdown`、`remark-breaks`、`rehype-sanitize`。
- AI 助手消息气泡不再使用 `whitespace-pre-wrap`，由 Markdown 组件控制段落间距。
- 后续如需要表格、任务列表等 GFM 特性，可再引入 `remark-gfm`。
