...
- **v2.4.0 回顾**：大纲横向卡片模式、时间轴事件地点字段、MC 主题素材丰富。
- **v2.3.0 回顾**：时间轴连线修复、MC 主题配色重构、文本模式可见性、设置教程、番茄钟联动。
- **v2.2.0 回顾**：时间轴/事件逻辑重写、动画与性能优化、MC 主题再设计、工作区保存改进、自定义字体导入。
- GitHub Release v2.4.0 已发布：<https://github.com/YJLZSL/Plotline/releases/tag/v2.4.0>
- **自动更新签名状态**：`TAURI_SIGNING_PRIVATE_KEY` GitHub Secret 已于 2026-06-26 配置。v2.5.0 发布时验证是否生成 `.sig` 文件。若 CI 未生成，需手动签名后上传。
