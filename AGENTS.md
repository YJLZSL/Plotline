...
### 当前迭代状态（v2.7.3 已发布：修复 v2.7.2 CI Rust 编译错误）
- **Rust 编译错误修复**：修复 v2.7.2 CI 中发现的 Rust 编译错误（`src-tauri/src/services/ai.rs` 中 `NeedApi` 字段匹配不完整、`UpdateWorkspaceInput` 字段缺失、`entities` 借用/移动问题），确保 CI/Release workflow 全绿。
...
- **版本号与文档**：`package.json`、`src-tauri/Cargo.toml`、`src-tauri/tauri.conf.json` 升级到 `2.7.3`；`AGENTS.md`、`更新日志.md`、`交接文档.md`、`docs/产品路线图.md` 更新至 v2.7.3 状态。
- **发布说明**：v2.7.3 的 Release 签名继续由 CI 自动完成，发布前仅需统一版本号与更新日志，无需手动维护 `releases/vX.Y.Z/latest.json` 签名。
- **GitHub Release v2.7.3**：<https://github.com/YJLZSL/Plotline/releases/tag/v2.7.3>

### 上一版本（v2.7.2 / v2.7.0 / v2.6.2 / v2.6.1 / v2.6.0 / v2.5.4 / v2.3.0 / v2.2.0 已发布/已标记）
...
> 文档版本：v0.5.7  
> 最后更新：2026-06-27（v2.7.3 已发布：修复 v2.7.2 CI Rust 编译错误）
