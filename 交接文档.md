...
> 代码与 tag 已推送，CI 已触发。下一任 AI 需要检查 CI 结果、整理仓库并重新确认/上传 Release 产物。

### 2.1 检查 CI 构建结果

- 查看 GitHub Actions 中 `release.yml` 是否成功。
- 若 CI 草稿 Release 已生成，下载其中的 `Plotline_2.2.0_x64-setup.exe` 与 `.sig`，与本地签名比对是否一致。

### 2.2 整理仓库

- 已重命名文档为中文名（保留 `AGENTS.md`、`README.md`），并新增 `docs/文档索引.md`。
- 请检查是否还有英文文档名残留、是否有重复/过期文档需要归档到 `docs/archives/`。
- 更新 `docs/文档索引.md` 中的状态列。
...
使用 `更新日志.md` 中 v2.2.0 章节内容，精简为 Release Notes。建议结构：
- 新增功能
- 改进与优化
- 修复
- 下载链接
