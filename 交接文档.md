...
> 代码与 tag 已推送，CI 已重新触发。当前 AI 已接手并完成以下步骤：检查 CI 结果、整理仓库、重新确认/上传 Release 产物。

### 2.1 检查 CI 构建结果

- ✅ 重新推送 tag `v2.2.0` 触发 CI（run #11），正在等待构建完成。
- 若 CI 成功，草稿 Release 会自动生成。
- 若 CI 再次失败，使用本地产物手动上传（见 2.3）。

### 2.2 整理仓库

- ✅ 已检查英文文档名残留：无残留，所有文档已中文化。
- ✅ 已检查重复/过期文档：`docs/archives/` 仅包含 `ISSUE_AUDIT_v1.5.md`，已归档。
- ✅ 已更新 `docs/文档索引.md` 中的状态列（交接文档标记为 ✅ 已发布）。

### 2.3 重新上传/确认 Release 产物
...
若 CI 未上传签名或 latest.json，手动上传本地产物：

```bash
gh release upload v2.2.0 src-tauri/target/release/bundle/nsis/Plotline_2.2.0_x64-setup.exe
gh release upload v2.2.0 src-tauri/target/release/bundle/nsis/Plotline_2.2.0_x64-setup.exe.sig
gh release upload v2.2.0 src-tauri/target/release/bundle/msi/Plotline_2.2.0_x64_en-US.msi
gh release upload v2.2.0 releases/v2.2.0/latest.json
```
...
- ✅ 已编写 `releases/v2.2.0/RELEASE_NOTES.md`，包含新增功能、改进优化、修复、构建产物与自动更新说明。

### 2.5 验证自动更新

- 待验证：安装 v2.1.0 旧版本 → 启动 → 应弹出 v2.2.0 更新提示。
