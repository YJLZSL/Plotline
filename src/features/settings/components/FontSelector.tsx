...
interface FontSelectorProps {
  ...
  /** B3: 外观 tab 只配置界面字体，编辑器 tab 只配置编辑器字体，避免重复挂载。 */
  variant?: 'both' | 'ui' | 'editor';
}

export function FontSelector({ uiFont, editorFont, fontTheme, onChange, variant = 'both' }: FontSelectorProps) {
  const { t } = useI18n();
  const enhancedAnimations = useUIStore((s) => s.enhancedAnimations);
  const showThemeCards = variant !== 'editor';
  const showUiPicker = variant !== 'editor';
  const showEditorPicker = variant !== 'ui';
